...
- [property_insurance.py](./property_insurance.py) - An inbound phone/WebRTC/local/chat example where an agent answers property insurance policy questions using RAG via the `DocumentQA` helper class.
...
- [restaurant_waitlist.py](./restaurant_waitlist.py) - An inbound phone/WebRTC/local/chat example where an agent collects caller name, party size, and callback number to add them to a restaurant waitlist using `call.set_task(...)`.
...
- [agent_testing.py](./agent_testing.py) - Demonstrates the three testing patterns: manual conversation with `session.say()`, LLM-driven roleplay via `agent.test_roleplay()`, and callback overriding with `agent.patch()`; includes pass/fail evaluation criteria.
